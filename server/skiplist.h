...
public:
    ...
    void loadString(std::string buf);
    int size();
public:
...
template<typename Key,typename Value>
std::string SkipList<Key,Value>::dumpString()
{
    std::string res;
    auto node=this->head->forward[0];
    while(node!=nullptr){
        res += node->key + DELIMITER + node->value + "\e";
        node=node->forward[0];
    }
    return res;
}
...
template<typename Key,typename Value>
void SkipList<Key,Value>::loadString(std::string buf)
{
    std::istringstream iss(buf);
    std::string line;
    std::string key;
    std::string value;
    while(std::getline(iss, line, '\e')){
        if(parseString(line,key,value)){
            addItem(key,value);
        }
    }
